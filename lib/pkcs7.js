...
p7.createSignedData = function() {
  ...
  function verifySignerInfos(mds, caStore, options) {
    ...
    for(var i = 0; i < msg.signers.length; ++i) {
      ...
      case forge.oids.sha1WithRSAEncryption:
        scheme = undefined; /* use PKCS#1 v1.5 padding scheme */
        break;
      case forge.oids['RSASSA-PSS']:
        var hash, mgf;

        /* initialize mgf */
        hash = forge.oids[signerCert.siginfo.parameters.mgf.hash.algorithmOid];
        if(hash === undefined || forge.md[hash] === undefined) {
          var error = new Error('Unsupported MGF hash function.');
          error.oid = signerCert.siginfo.parameters.mgf.hash.algorithmOid;
          error.name = hash;
          throw error;
        }

        mgf = forge.oids[signerCert.siginfo.parameters.mgf.algorithmOid];
        if(mgf === undefined || forge.mgf[mgf] === undefined) {
          var error = new Error('Unsupported MGF function.');
          error.oid = signerCert.siginfo.parameters.mgf.algorithmOid;
          error.name = mgf;
          throw error;
        }
        ...
        hash = forge.oids[signerCert.siginfo.parameters.hash.algorithmOid];
        if(hash === undefined || forge.md[hash] === undefined) {
          throw {
            message: 'Unsupported RSASSA-PSS hash function.',
            oid: signerCert.siginfo.parameters.hash.algorithmOid,
            name: hash
          };
        }

        scheme = forge.pss.create(forge.md[hash].create(), mgf,
                                  signerCert.siginfo.parameters.saltLength);
        break;
      }

      // verify digest
      var verified = signerCert.publicKey.verify(signer.md.digest().bytes(), signer.signature, scheme);
      rval = rval && verified;
